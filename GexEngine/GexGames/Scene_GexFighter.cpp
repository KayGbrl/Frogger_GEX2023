...
void Scene_GexFighter::playerMovement() {
    ...
    if (m_player->hasComponent<CTransform>())  {
        auto playerP = m_player->getComponent<CTransform>().pos.y;
...
void Scene_GexFighter::sCollisions() {
    ...
    //Check for collision with pickup item
...
void Scene_GexFighter::checkIfDead(NttPtr e) {
    ...
    if (e->hasComponent<CHealth>()) {
        if (e->getComponent<CHealth>().hp <= 0) {
            ...
            SoundPlayer::getInstance().play("Explosion2");
        }
    }
}
...
void Scene_GexFighter::sRender() {
    ...
    if (m_isPaused) {
        sf::Text paused("PAUSED", m_game->assets().getFont("Mario"), 145);
        centerOrigin(paused);
    ...
    if (m_LossePlayer) {
        sf::Text msg("You Lost!", m_game->assets().getFont("Mario"), 145);
        centerOrigin(msg);
        auto bounds = getViewBounds();
    ...
    if (m_winplayer) {
        sf::Text msg("You WON!", m_game->assets().getFont("Mario"), 145);
        centerOrigin(msg);
        auto bounds = getViewBounds();
        msg.setPosition(bounds.left + bounds.width / 2.f, bounds.top + bounds.height / 2.f);
        m_game->window().draw(msg);
...
{
    if (e->hasComponent<CTransform>()) {
        auto spawn = e->getComponent<CTransform>().pos;
        std::uniform_int_distribution t(1, 4);
        //Make the pickup random in game
        auto pickupTaken = t(rng);

        std::string pickupTexture;
        std::string category;

        switch (pickupTaken) {
        case 1: 
            pickupTexture = "FireRate";
            category = "fireRatePickup";
            break;

        case 2:
            pickupTexture = "FireSpread";
            category = "fireSpreadPickup";
            break;

        case 3:
            pickupTexture = "HealthRefill";
            category = "healthPickup";
            break;

        case 4:
            pickupTexture = "MissileRefill";
            category = "missilePickup";
            break;
        }

        //Create pickup
        auto pickedup = m_entityManager.addEntity(category);
        pickedup->addComponent<CCollision>(10.f);
        pickedup->addComponent<CTransform>(spawn, sf::Vector2f(4.f, 9.f));
        //Add pickup texture
        auto texture = m_game->assets().getAnimation(pickupTexture);
        pickedup->addComponent<CAnimation>(texture);
    }
}
